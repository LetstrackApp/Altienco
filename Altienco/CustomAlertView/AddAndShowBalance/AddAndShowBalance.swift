...
class AddAndShowBalance: UIControl,ViewLoadable {
    ...
    @IBOutlet weak var addAmountButton: UIButton!{
        didSet {
            addAmountButton.contentMode = .center

            addAmountButton.titleLabel?.textAlignment = .center
            addAmountButton.titleLabel?.numberOfLines = 2
            addAmountButton.titleLabel?.adjustsFontSizeToFitWidth = true
            addAmountButton.titleLabel?.minimumScaleFactor = 0.5
            addAmountButton.layer.cornerRadius = 6
            addAmountButton.titleLabel?.setCharacterSpacing(1.6)
    ...
    func onLanguageChange() {
        ...
        addAmountButton.setTitle(lngConst.add_Balance, for: .normal)
    }
